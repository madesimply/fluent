...
const api = {
  ...
  posts: {
    get(this: { title?: string }, data: any, chain: { chain: any }) {
...
// This will now show a type error because no argument is provided
const chain1 = root.users.get('test');

// This is correct and type-safe
const chain2 = root.posts.get({ chain: ['test', 1] });
type Chain2Type = typeof chain2.chain;

// This will create a chain with literal types
const chain3 = root.posts.get({ chain: root.users.get('test') });
const result = chain3.run(1);

// Type of chain3.chain